...
class Utils
{
     ...
     */
    public static function displaySingleItemByID($id, $callback = null, array $params = [])
    {
        global $post;
        $post = get_post($id);
        if (!is_null($post)) {
            setup_postdata($post);
            if ($callback !== null)
                call_user_func_array($callback, array_merge([$post], $params));
            wp_reset_postdata();
        }
        return $post;
    }
     ...
     * @return \WP_Query
     */
    public static function displayItems($post_type = 'any', array $args = [], $callback = null, array $params = [])
    {
